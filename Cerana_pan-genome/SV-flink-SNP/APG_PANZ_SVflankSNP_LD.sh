...
OPTIONS: ([R]:required  [O]:optional)
    ...
    -q, --query      <str>    [R]    Input query SV start of vcf file, bcf or gz and indexed (SV)
    -q2, --query2    <str>    [R]    Input query SV end of vcf file, bcf or gz and indexed (SV)	
...
check_files_exists $query $query2 $ref $rscript
check_var_empty query query2 ref
...
Each_record_dir :
    ${pre}/LDs   : dir of each query and query2 LDs
    ${pre}/Rcalc : dir of each query and query2 types (and figs if mode not 0)
------------------------------------------------------------
"
# >>>>>>>>>>>>>>>>>>>>>>>> Checking >>>>>>>>>>>>>>>>>>>>>>>>
md5_query_sample=$( bcftools view -h $query | tail -n 1 | md5sum | cut -d " " -f 1 )
md5_query2_sample=$( bcftools view -h $query2 | tail -n 1 | md5sum | cut -d " " -f 1 )
...
if [ "$md5_query2_sample" != "$md5_ref_sample" ]; then
    gst_err "Query2 and Ref file should have same sample orders."
    exit 1
fi
# ? mk output dirs
mkdir -p $pre &&\
...
if [[ -s "${pre}/merge_idx.tmp" && -s "${pre}/merge_idx-Query.tmp" && -s "${pre}/merge_idx-Query2.tmp" && -s "${pre}/merge_idx-Ref.tmp" ]];then
    ...
    ' > ${pre}/query_idx.tmp &&\
    csvtk cut -tTH -j $threads -f 1,2,3 $query2 | perl -lane '
        BEGIN{$type="Query2";$,="\t";}
        $ord_in_chr=++$h{$F[0]};
        print "$_\t$ord_in_chr\t$type";
    ' > ${pre}/query2_idx.tmp &&\
    ...
    cat ${pre}/query_idx.tmp ${pre}/query2_idx.tmp ${pre}/ref_idx.tmp | csvtk sort -tTH -j $threads -k 1 -k 2:n |perl -lane '
        BEGIN{$,="\t";}
        $ord_in_chr=++$h{$F[0]};
        print "$_\t$ord_in_chr";
    ' > ${pre}/merge_idx.tmp &&\
...
function Calc_LD () {
    ...
    query_rcd2=$(grep -m 1 -w "$query_id" ${pre}/merge_idx-Query2.tmp)
    query_ord=$(echo "$query_rcd" | grep 'Query' | cut -f 6)
    query_ord2=$(echo "$query_rcd2" | grep 'Query2' | cut -f 6)
    query_chr=$(echo "$query_rcd" | grep 'Query' |cut -f 1)
    ...
    ref_just_after_query=$(
        cat ${pre}/merge_idx-Ref.tmp |\
        csvtk grep -tTH -p "$query_chr" -f 1 |\
        csvtk filter -tTH -f "6>$query_ord2" |\
        head -n 1
    ) 
    ...
    if [[ "$ref_before_num" -lt "$size" || "$ref_after_num" -lt "$size" ]];then  
        ...
        #before SV
        before_ref_records=$( echo -e "${ref_before_records}")
        ref_id=$( echo "$before_ref_records" | cut -f 3 )
        before_ref_region=$( echo "$before_ref_records" | cut -f 1,2 | sed -n '1p;$p' | sed 'N;s/\n/\t/' | perl -F"\t" -lane 'print "$F[0]:$F[1]-$F[3]";')
		#after SV
        after_ref_records=$( echo -e "${ref_after_records}")
        ref_id2=$( echo "$after_ref_records" | cut -f 3 )
        after_ref_region=$( echo "$after_ref_records" | cut -f 1,2 | sed -n '1p;$p' | sed 'N;s/\n/\t/' | perl -F"\t" -lane 'print "$F[0]:$F[1]-$F[3]";')
		#sed -n '1p;$p' 
		######################################################################################################################################################################################
		#               
		######################################################################################################################################################################################
		#                                                     The next step is to obtain vcf genotype information for all SNPS and SVS in this range
		######################################################################################################################################################################################
        #? get ref vcf and format them: change format to "."
        before_ref_vcf=$(
            bcftools view -H -r $before_ref_region $ref 2>/dev/null |\
            ...
            ')
        after_ref_vcf=$(
            bcftools view -H -r $after_ref_region $ref 2>/dev/null |\
            csvtk grep -tTH -f 3 -P <(echo "$ref_id2") |\
            perl -F"\t" -lane '
                BEGIN{$,="\t";}
                $F[7]=".";
                print @F;
        ...
        # gst_log "44"
        merge_vcf=$(echo -e "${before_ref_vcf}\n${query_vcf}\n${after_ref_vcf}" | sort -k1,1 -k2,2n | perl -lane '$F[1]=$.;$_=join("\t",@F);print;' ) 
